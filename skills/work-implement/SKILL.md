...
1. **Contract** — how we'll know the whole thing worked, in plain words: which tests, what gets observed running, what the human will look at. User-facing acceptance criteria as plain sentences. Also record `LOOP_BASE`, the UTC opening time, a whole-loop budget in hours set by the human before the first dispatch, and explicit `Production paths` and `Test paths` used to count added lines.
...
**Dispatch.** Check the whole-loop budget, then append `task <name> dispatched <UTC>; budget <N>h; deadline <UTC>` to Breadcrumbs; a task gets two wall-clock hours unless the human set another budget. Record BASE (`git rev-parse HEAD`) and spawn a fresh implementer sized to the task: cheap and fast for a mechanical one-or-two-file task with a complete spec, standard for multi-file integration, most capable only where design judgment lives. The dispatch contains: the task's own lines from `progress.txt` (sentence, *done when*, *check by*); the contract; the absolute task deadline; paths to the plan, `CONTEXT.md`, and `CONSTITUTION.md`; interfaces from earlier tasks it couldn't discover; and the reply contract — implement, test, commit, append what it did, what it ran, and any gotchas to Breadcrumbs, and return one status: **done / done-with-concerns / needs-context / blocked**. The brief requires the implementer to stop at the deadline, append its current edits, commits, checks, and open work to Breadcrumbs, and return **blocked**; it must not start a command that can outlive the remaining budget. Never paste session history into a dispatch — a fresh agent needs its task, not your story. Have implementers use `/tdd` (from mattpocock/skills) at the seams the plan agreed — invoked, not paraphrased, when it's installed; only when it's absent does the dispatch carry its essence instead: red before green, one slice at a time, tests only at agreed public seams, never against internals. Before every later dispatch, compare the current UTC time with both deadlines. If either budget has expired without a clean review, dispatch nothing: update `handoff.md` and surface elapsed time, commits, checks, open findings, and the next decision to the human.
...
**Complete.** Check the box, append one line — `done (commits <base>..<head>, review clean)` or `…, N parked` — and move on. Never advance past open blocking findings. Compare cumulative added lines in the Contract's recorded `Test paths` and `Production paths` over `LOOP_BASE..HEAD`. The signal fires only when both categories have additions and test additions are at least three times production additions: append the counts and surface the ratio as a non-blocking signal in the next human update. Below the ratio, or with no additions in either category, record the counts and no signal. Never lets it create another fix round by itself.

## The ship gate

When every box is checked: one fresh reviewer on the most capable model makes one pass over the whole branch diff against the contract and triages the parked, minor, and newly found lines. Every non-blocking finding that still stands becomes its own GitHub issue on the repository, with a plain-words title, the `parked` label, and a body containing its written ruling and concise evidence; create the repository label once if it is absent, and never bundle findings. Dispatch no fixer and no re-review from the ship gate. Any residual finding that meets the blocking definition goes to the human. Then the human looks at what the contract promised they would look at; their reaction, not the reviews, is the verdict.
