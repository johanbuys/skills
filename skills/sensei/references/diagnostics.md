...
15. Parametrised by `learner.home_lang`, `learner.work_stack`, and the capstone domain if any.

## diag-1 · debug (`home_lang`)
Generate `<dojo>/katas/<date>-diag-debug/`: a ~60-line module in a small domain with **two** planted
...
## diag-2 · review (the work stack)
A ~50-line diff shaped like the work stack (e.g. a controller + service + DTO) with **five** planted
