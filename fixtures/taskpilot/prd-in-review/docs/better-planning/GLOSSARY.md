...
**Notifiable event** — a store condition the notifications feature may surface: an occurrence
becoming due, or overdue past a grace period. The v1 set is decision N3 — **open**.
· *Not* every store mutation; generation itself is silent.
· Specced: notifications-prd.md → Requirements, Decision N3 (open).

**Snooze** — suppressing re-notification for one occurrence for a chosen interval. Pure
notification state; never mutates the occurrence itself.
· *Not* skip (which marks the occurrence) and *not* pause (which acts on the template).
· Specced: notifications-prd.md → Requirements.

**Quiet hours** — a delivery-suppression window; suppressed notifications coalesce into the
next allowed window rather than being dropped.
· *Not* a do-not-generate window — generation is unaffected.
· Specced: notifications-prd.md → Requirements, Decision N5 (proposed).
