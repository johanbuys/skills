...
| [`sensei`](skills/sensei/SKILL.md) | A daily twenty-minute hand-coding practice session the agent runs and steers, wrapped in a small e-learning course that lives in one git repo (the dojo). Each ladder rung gets a lesson written the night before — outcomes, concept prose, the docs section, a course video with timestamps — read outside the box; the session opens with a three-question quiz on it, then a five-minute drill on whatever is due across your tracks, then a kata you write by hand as the next brick of a real capstone app at `capstone/<name>/` that runs and answers a curl, then explain-it-back, and a learner model in `.dojo/` updated by rule tables. Tracks are learn (one at a time) or sharpen (reviewed as it decays); `setup <track>` researches a ladder with resources per rung from the official docs plus real courses, and builds the capstone shell. An optional page served by `scripts/serve` shows the course sidebar (ladder, capstone bricks, what is due, attendance), the lesson, the clock, the click-to-comment diff review and the check button. Sends tangents to `study`. No hooks — the clock rides on `./check` and the write guard is git authorship, so it behaves the same in any harness. |
